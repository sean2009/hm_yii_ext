...
class cls_session
{
    ...
    {
    	$sql ='SELECT userid, adminid, user_name, user_rank, discount, email,session_data, expiry FROM ' . $this->session_table . " WHERE sesskey = '" . $this->session_id . "'";
    ...
    function destroy_session()
    {
        ...
        /* ECSHOP 自定义执行部分 */
        if (!empty(Yii::app()->ecs))
        {
            $this->db->query('DELETE FROM ' . Yii::app()->ecs->table_oci('cart') . " WHERE session_id = '$this->session_id'");
        }
        /* ECSHOP 自定义执行部分 */
