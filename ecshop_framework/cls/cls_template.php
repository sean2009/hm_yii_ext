...
class cls_template {
	...
	/**新添加**/
	var $is_memcached = true;//针对后台自定义模版是否启用memcache
	var $memcache_time = 60;//针对后台自定义模版memcache缓存时间
	var $is_cached = false;//是否重新编译文件
	...
	function fetch($filename, $cache_id = '') {
		...
		if (strncmp($filename, 'str:', 4) == 0) {
			$filename = ROOT_PATH . substr($filename, 4);
	...
	function make_compiled($filename) {
		...
		if ($this->is_cached && time() > $expires + $this -> cache_lifetime) {
			$this -> _current_file = $filename;
		...
		}else if ($this -> force_compile || $filestat['mtime'] > $expires) {
			$this -> _current_file = $filename;
			$source = $this -> fetch_str(file_get_contents($filename));
		...
		if (!empty($moddb)) {
			foreach ($moddb AS $key => $mod) {
				...
				switch ($s[0]) {
					...
					case 'truncate' :
						$p = '$this->sub_str(' . $p . ",$s[1])";
						break;
					case 'number_format' ://数字格式化
						$s[1] = empty($s[1]) ? 2 : $s[1];
						$p = 'number_format(' . $p . ",$s[1])";
						break;
					case 'strip_tags' :
						$p = 'strip_tags(' . $p . ')';
						break;
					...
					case 'img_press' :
						$p = '$this->img_press(' . $p . ')';
						break;
					case 'img_flash_press' :
						$p = '$this->img_flash_press(' . $p . ')';
						break;
					default :
					# code...
						break;
...
//			}
	...
	function img_press($str){
		$path = $this->img_url($str);
		$index = strrpos($path,'.');
		
		return strpos($path, 'infoDefault') === false ? (substr($path,0,$index).'_300225'.substr($path,$index,10)) : $path;
	}
	
	function img_flash_press($str){
		$path = $this->img_url($str);
		$index = strrpos($path,'.');
	...
	function img_url($str)
    {
    	...
    	if(strlen($str)>0)
    	{
			...
			else 
			{
				...
				{
					$str = $GLOBALS['img_servers'][array_rand($GLOBALS['img_servers'])]. str_replace($replaceUrl, '', $str);
				}
			}
    	}
    	if(empty($str))
    	{
    		$str = CSS_DOMAIN1.'/images/public/infoDefault.jpg';
    	}
    	return $str;
    }
	/**
	 ...
	 */
	function action($str){
		if(preg_match_all('/{action:(.*?)}/i', $str, $result))
		{				
			if(isset($result))
			{
				$actions = $result[0];
				...
				{			
					$eventid = $action_ids[$key];
                    if (strpos($eventid, '$') === 0) {
                        $eventid = $this->_var[substr($eventid, 1)];
                    }
					//$source = $this->get_box($eventid);
					$source = '<?php echo $this->get_box('.$eventid.'); ?>';
					$str = str_replace($action, $source,$str);
				}
			}
		}
		return $str;
	 ...
	 * @param int $id
	 */
	function get_box($key){
		try {
			$sql = 'SELECT CACHE_OUT_HTML FROM CMS_CACHE WHERE CACHE_KEY=:CACHE_KEY AND IS_DELETED=0';
			Yii::app()->dbc -> bind('CACHE_KEY',$key);
			return Yii::app()->dbc -> getOne($sql,$this->is_memcached,$this->memcache_time,'cms_'.$key);
		}
		catch (Exception $err){
			return '';
		}
	 ...
	 * 生成HTML代码
	 * @author wander
	 * @param string $source 模版源码
	 */
	function display_html($source) {
		echo $this->create_html($source);
	}
	/**
	 * 生成HTML代码
	 * @author wander
