...
abstract class EMongoDocument extends EMongoEmbeddedDocument {
    ...
    public function insert(array $attributes = null) {
        ...
        if ($this->beforeSave()) {
            ...
            $monitor->log(MonitoringService::LOG_TYPE_MONGO, array(
    ...
    public function deleteByPk($pk, $criteria = null) {
        ...
        $monitor->log(MonitoringService::LOG_TYPE_MONGO, array(
            'type' => 'REMOVE',
            'content' => json_encode($criteria->getConditions()),
            'et' => MonitoringService::diffMillisecond(MonitoringService::getMillisecond(), $t1)
        ));
    ...
    public function deleteAll($criteria = null) {
        ...
        if (version_compare(Mongo::VERSION, '1.0.5', '>=') === true)
            $ret = $this->getCollection()->remove($criteria->getConditions(), array(
                    'justOne' => false,
                    'fsync' => $this->getFsyncFlag(),
                    'safe' => $this->getSafeFlag()
            ));
        else
            $ret = $this->getCollection()->remove($criteria->getConditions(), false);
