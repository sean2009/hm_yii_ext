...
class ImageUtil {
	...
	function __construct($bgcolor = '') {
		$this->ImageUtil ( $bgcolor );
	}
	
	function ImageUtil($bgcolor = '') {
		if ($bgcolor) {
	 ...
	 */
	function make_thumb($img, $tid = 'big', $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor = '') {
		...
		if (! $org_info) {
			$this->error_msg = sprintf ( 'missing_orgin_image', $img );
		...
		if (! $this->check_img_function ( $org_info [2] )) {
			$this->error_msg = sprintf ( 'nonsupport_type', $this->type_maping [$org_info [2]] );
	   ...
	   if(($org_info [0] <$thumb_width) && ($org_info [1] < $thumb_height)){
		...
		$imageType = strtolower ( substr ( image_type_to_extension ( $org_info [2] ), 1 ) );
		$imageType = $imageType == 'jpg' ? 'jpeg' : $imageType;
		$imageType = "jpeg";//统一生成jpeg格式图片
		$filename = $tid.'.'.$imageType;
		$imageFun = 'image' . ($imageType == 'jpg' ? 'jpeg' : $imageType);
		$imageFun ( $img_thumb, $dir . '/'.$filename);
	...
	function add_watermark($filename, $target_file = '', $watermark = '', $watermark_place = '', $watermark_alpha = 0.65) {
		...
		if ((! file_exists ( $filename )) || (! is_file ( $filename ))) {
			$this->error_msg = sprintf ( 'missing_orgin_image', $filename );
			$this->error_no = ERR_IMAGE_NOT_EXISTS;
		...
		if (! $watermark_handle) {
			$this->error_msg = sprintf ( 'create_watermark_res', $this->type_maping [$watermark_info [2]] );
		...
		if (! $source_handle) {
			$this->error_msg = sprintf ( 'create_origin_image_res', $this->type_maping [$source_info [2]] );
			$this->error_no = ERR_INVALID_IMAGE;
	...
	function validate_image($path) {
		if (empty ( $path )) {
			$this->error_msg = 'empty_watermark';
		...
		/* 文件是否存在 */
		if (! file_exists ( $path )) {
			$this->error_msg = sprintf ( 'missing_watermark', $path );
			$this->error_no = ERR_IMAGE_NOT_EXISTS;
			return false;
		}
		...
		if (! $image_info) {
			$this->error_msg = sprintf ( 'invalid_image_type', $path );
			$this->error_no = ERR_INVALID_IMAGE;
			return false;
		}
		...
		if (! $this->check_img_function ( $image_info [2] )) {
			$this->error_msg = sprintf ( 'nonsupport_type', $this->type_maping [$image_info [2]] );
			$this->error_no = ERR_NO_GD;
			return false;
		}
	...
	function unique_name($dir) {
		...
		while ( empty ( $filename ) ) {
			$filename = ImageUtil::random_filename ();
